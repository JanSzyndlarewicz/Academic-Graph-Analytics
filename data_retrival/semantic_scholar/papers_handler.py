...
def fetch_papers_by_dois(dois, fields):
    try:
        papers = ss_api.get_papers(dois, fields=fields)
        return [paper_to_dict(paper) for paper in papers]
    ...
    for json_obj in json_iterator:
        ...
        if doi:
            ...
            if len(chunk) == chunk_size:
                papers_dict = fetch_papers_by_dois(chunk, fields=["citations.externalIds", "externalIds"])
